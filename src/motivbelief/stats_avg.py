...
def summarize_between_groups(coef_df: pd.DataFrame, group_col: str = "choiceType") -> pd.DataFrame:
    if coef_df.empty:
        return pd.DataFrame(columns=["pair", "coef", "mean", "se", "t", "df", "p"])
    coef_cols = [c for c in coef_df.columns if c not in ("participant", group_col)]
    ...
    for i, g1 in enumerate(group_levels):
        for g2 in group_levels[i + 1 :]:
            ...
            for cn in coef_cols:
                x1 = d1[cn].to_numpy(dtype=float)
                x2 = d2[cn].to_numpy(dtype=float)
                ...
                rows.append(
                    {
                        "pair": f"{g2} - {g1}",
                        "coef": cn,
                        "mean": float(np.mean(x2) - np.mean(x1)),
                        "se": float(np.sqrt(np.var(x1, ddof=1) / x1.size + np.var(x2, ddof=1) / x2.size)),
                        "t": float(tt.statistic),
                        "df": float(tt.df),
                        "p": float(tt.pvalue),
                    }

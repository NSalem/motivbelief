...
def stats_avg_root_nonfree(repo: Path) -> Path:
    """Average-level stats for Non-Free choices fitted per-condition."""
    return _stats_branch(repo, "average_nonfree")


def stats_trial_root_nonfree(repo: Path) -> Path:
    """Trial-level stats for Non-Free choices fitted per-condition."""
    return _stats_branch(repo, "trial_nonfree")


def fig_dir(repo: Path) -> Path:
    """Output directory for all `fig_*` figures (`<repo>/plots`)."""

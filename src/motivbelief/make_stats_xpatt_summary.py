...
from motivbelief.stats_xpatt import UMINUS, combine5
...
def build_table4_free_nonfree_between(trial_root: Path, digits: int = 2) -> str:
    """Welch Free vs Non-Free contrast for merged data and each simulation stem."""
    contrast = f"Free {UMINUS} Non-Free"
    stems: List[Tuple[str, Path]] = [
        ("Data (1a, 2 free, 3)", trial_root / "exp1a_exp2free_exp3" / "between_groups_conf_xpatt_pairwise_welch.csv"),
        ("Act model", trial_root / "sim_act" / "between_groups_conf_xpatt_pairwise_welch.csv"),
        ("Intent model", trial_root / "sim_intent" / "between_groups_conf_xpatt_pairwise_welch.csv"),
        ("Confirm model", trial_root / "sim_confirm" / "between_groups_conf_xpatt_pairwise_welch.csv"),
    ]
    ...
    return _html_table_simple(headers, rows)

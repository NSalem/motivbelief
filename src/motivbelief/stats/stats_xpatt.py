...
def tidy_pairwise_group(effects_all: pd.DataFrame, eff: str, label_effect: str) -> pd.DataFrame:
    """Between-group contrast (e.g. Free vs Non-Free) for one coefficient.

    Uses a paired t-test whenever the two groups turn out to be the exact
    same set of participants (one row each, no duplicates) -- the case for
    our model simulations, where a single fitted model per subject drives
    both the Free and the Non-Free predictions, so the comparison is
    inherently within-subject. Falls back to a Welch two-sample t-test
    otherwise, which is the correct test for the real data (Free and
    Non-Free participants there are disjoint subject pools). The point
    estimate (mean difference) is identical either way; only se/t/df/p/CI
    differ, since the paired test removes between-subject variance that
    is irrelevant to how consistently a given model's own predictions
    shift between conditions.

    The paired-vs-Welch decision is made from each group's FULL participant
    roster in ``effects_all`` -- not from the subset that happens to have a
    non-NaN value for this particular ``eff`` -- since cohort identity
    (same simulated/real subjects or not) doesn't depend on which effect
    we're testing. Otherwise a handful of participants with a NaN ``eff``
    in only one of the two conditions (e.g. too few simulated Error trials
    at some coherence to fit that participant's slope) would silently
    ...
    """
    grps = [g for g in GROUP_LEVELS if g in set(effects_all["group"])]
    if len(grps) < 2:
        return pd.DataFrame(columns=["Effect", "Contrast", "estimate", "se", "t", "df", "p", "lcl", "ucl", "test"])
    rows = []
    for g1, g2 in itertools.combinations(grps, 2):
        p1_full = effects_all.loc[effects_all["group"] == g1, "participant"]
        p2_full = effects_all.loc[effects_all["group"] == g2, "participant"]
        cohort_paired = (
            len(p1_full) > 1
        ...
        sub1 = effects_all.loc[effects_all["group"] == g1, ["participant", eff]].copy()
        sub2 = effects_all.loc[effects_all["group"] == g2, ["participant", eff]].copy()
        sub1[eff] = pd.to_numeric(sub1[eff], errors="coerce")
        sub2[eff] = pd.to_numeric(sub2[eff], errors="coerce")
        sub1 = sub1[np.isfinite(sub1[eff])]
        sub2 = sub2[np.isfinite(sub2[eff])]
        if sub1.empty or sub2.empty:
            continue

        if cohort_paired:
            # Available-case: pair on whichever participants have a finite
            # value in BOTH conditions for this effect, rather than
            # requiring the entire cohort to survive.
            merged = sub1.merge(sub2, on="participant", suffixes=("_1", "_2"))
            diff = (merged[f"{eff}_1"] - merged[f"{eff}_2"]).to_numpy(dtype=float)
            ...
            x1 = sub1[eff].to_numpy(dtype=float)
            x2 = sub2[eff].to_numpy(dtype=float)
            if len(x1) < 2 or len(x2) < 2:
                continue
            tt = stats.ttest_ind(x1, x2, equal_var=False)
            est = float(np.mean(x1) - np.mean(x2))
...
def make_tidy_group_tables(
    ...
    between_rows = []
    for code in effect_codes:
        between_rows.append(tidy_pairwise_group(effects_all, code, effect_labels[code]))
    between = pd.concat(between_rows, ignore_index=True) if between_rows else pd.DataFrame()
    if between.empty:
        between = pd.DataFrame(columns=["Effect", "Contrast", "estimate", "se", "t", "df", "p", "lcl", "ucl", "test"])
    else:
        between = between[["Effect", "Contrast", "estimate", "se", "t", "df", "p", "lcl", "ucl", "test"]]
...
def render_md_summary(
    ...
    return (
        ...
        f"## Between-groups (paired or Welch t-test per row, see `test` column)\n\n{tbl_between}\n"
    )
...
def run_one_stem(stem: str, df: pd.DataFrame, outdir: Path) -> None:
    ...
    between_c.to_csv(outdir / "between_groups_conf_xpatt_ttests.csv", index=False, na_rep="")
    ...
    if "log_rt" in df.columns:
        ...
        between_l.to_csv(outdir / "between_groups_logrt_xpatt_ttests.csv", index=False, na_rep="")
        ...
        between_cc.to_csv(outdir / "between_groups_conf_ctrlrt_xpatt_ttests.csv", index=False, na_rep="")
...
def main() -> None:
    if __package__ in (None, ""):
        _bootstrap_path()
    from motivbelief.stats.stats_avg import build_analysis_specs
